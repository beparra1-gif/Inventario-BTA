...
// Mismo contenido que usaba el rol "auditor" en su propio login separado,
// ahora embebido tal cual dentro del panel de admin/superadmin — así no
// hace falta crear ni entrar con otra cuenta para buscar cualquier
// inventario, revisar tax por tax y validar. El backend (routes/auditoria.js)
// ya acepta cualquier cuenta válida de admins, no un rol específico.
export function AuditoriaPanel({ adminId, onVolver }) {
  ...
  async function cargarInventarios() {
    try {
      setInventarios(await api.buscarInventarios(busqueda, adminId));
    } catch {
  ...
  async function abrirInventario(inv) {
    ...
    setCargando(true);
    try {
      setTaxes(await api.auditoriaTaxesDeInventario(inv.id, adminId));
    } catch {
  ...
  async function abrirTax(t) {
    ...
    setCargando(true);
    try {
      const d = await api.auditoriaDetalleTax(t.id, adminId);
      setDetalle(d);
  ...
  async function confirmarValidacion() {
    ...
    try {
      const r = await api.auditoriaValidarTax(taxSel.id, adminId, cantidad);
      mostrarToast(
        r.inconsistente
  ...
  return (
    <div className="tarjeta" style={{ maxWidth: 560 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        <h3 style={{ margin: 0 }}>Auditoría</h3>
        {onVolver && <button className="btn-texto" style={{ padding: 0 }} onClick={onVolver}>‹ Volver</button>}
      </div>
      <p style={{ fontSize: 12, color: 'var(--texto-tenue)', margin: '-4px 0 10px' }}>
        Busca cualquier inventario para revisar sus tax cerrados y confirmar (o marcar inconsistencia en) lo que se capturó.
      </p>
      <input
        className="campo"
